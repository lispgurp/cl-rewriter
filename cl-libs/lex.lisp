...
modified by setting the appropriate variables in the cl-ppcre regex library."
  (let (regexes              ;;; Gurp: nil default let binding
        forms                ;;; Gurp: nil default let binding 
        (registers (list 0)) 
        register-names       ;;; Gurp: nil default let binding 
        (used-register-names (make-hash-table :test #'equal))) 
      (dolist (pattern patterns)
	(multiple-value-bind (regex form)
        (if (consp pattern) ;;; Gurp: pattern is a list where (first lst) is the actual regular expression and (rest lst) is a list of executable forms
	  ...
	  (push (+ (car registers)
		   (progn
		     ...
		     (do ((i 0 (1+ i)) 
                  (open-backslash nil (and (null open-backslash) (char= (char regex i) #\\))) ;open-backslash true if there "was" no open backslash and there "is" one now
			  (open-character-set nil (case (char regex i)                                    ;open-character-set is true if
						    (#\[ (or open-character-set (not open-backslash)))                ; there "is" an open character now and (there "was" and open character or there "was" no open backslash
						    (#\] (and open-backslash open-character-set))                     ; there "is" a closed character and there "was" an open backslash and there "was" an open character
